...
| Phase 2 | MCP Server | 🔲 待开始 |
| Phase 3 | 增强功能（封面图、鉴权、清理等） | 🔲 待开始 |
...
### Phase 2 - MCP Server（下一步）

**目标**：让 AI 客户端能通过 MCP 协议直接调用服务

**待实现**：
- [ ] MCP Server（SSE 传输）
- [ ] 工具注册：create_article、update_article、upload_image、publish_to_draft、get_article、list_articles
- [ ] 挂载到 FastAPI（/mcp 端点）
